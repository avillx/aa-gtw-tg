...
class RecordingHandler(event.EventHandler):
    ...
    def process_loop_exit(self, event: models.LoopExitEvent) -> None:
        self.calls.append(("loop_exit", event))

    def process_compaction(self, event: models.CompactionEvent) -> None:
        self.calls.append(("compaction", event))

    def process_completion(self, event: models.CompletionEvent) -> None:
        self.calls.append(("completion", event))

    def process_completion_mistake(self, event: models.CompletionMistakeEvent) -> None:
        self.calls.append(("completion_mistake", event))

    def process_tool_error(self, event: models.ToolErrorEvent) -> None:
        self.calls.append(("tool_error", event))

    def process_tool_result(self, event: models.ToolResultEvent) -> None:
        self.calls.append(("tool_result", event))

    def process_error(self, err: models.Error) -> None:
        self.calls.append(("error", err))
...
def test_completion_event_parsed() -> None:
...
def test_malformed_data_json_raises() -> None:
    with pytest.raises(ValueError):
    ...
    [
        (
            models.CompletionEvent(
                type_=models.CompletionEventType.COMPLETE, done=False, completion="x", tool_calls=[]
            ),
            "completion",
        ),
        (
            models.CompletionMistakeEvent(
                type_=models.CompletionMistakeEventType.COMPLETE_MISTAKE, error="e"
            ),
            "completion_mistake",
        ),
        (
            models.CompactionEvent(
                type_=models.CompactionEventType.COMPACTION, message="m", result="r"
            ),
            "compaction",
        ),
        (models.LoopExitEvent(type_=models.LoopExitEventType.LOOP_EXIT), "loop_exit"),
        (
            models.ToolResultEvent(type_=models.ToolResultEventType.TOOL_RESULT, id="i", result=[]),
            "tool_result",
        ),
        (
            models.ToolErrorEvent(
                type_=models.ToolErrorEventType.TOOL_ERROR, cause="c", tool_name="t", args=""
            ),
            "tool_error",
        ),
...
def test_process_event_dispatches(ev: event.EventType, method: str) -> None:
...
def test_process_event_unknown_raises() -> None:
    aeh = event.AgentEventHandler(
        client=CLIENT, event_handler=RecordingHandler(), provided_tools=[]
    )
...
def test_allowed_tool_servers() -> None:
    ...
    aeh = event.AgentEventHandler(
        client=CLIENT, event_handler=RecordingHandler(), provided_tools=[t1, t2]
    )
    ...
    assert schema.to_dict() == {"type": "object", "properties": {}}


def test_allowed_tool_servers_empty() -> None:
    aeh = event.AgentEventHandler(
        client=CLIENT, event_handler=RecordingHandler(), provided_tools=[]
    )
...
def test_provided_toolcall_known(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    aeh = event.AgentEventHandler(
        client=CLIENT, event_handler=RecordingHandler(), provided_tools=[t]
    )
...
def test_provided_toolcall_exec_error(monkeypatch: pytest.MonkeyPatch) -> None:
    sync_mock = unittest.mock.Mock()
    monkeypatch.setattr(event.tool_result, "sync", sync_mock)

    aeh = event.AgentEventHandler(
        client=CLIENT, event_handler=RecordingHandler(), provided_tools=[FailingTool(name="boom")]
    )
    ev = models.ProvidedToolCallEvent(
        type_=models.ProvidedToolCallEventType.PROVIDED_TOOLCALL,
        tool="boom",
...
def test_provided_toolcall_unknown(monkeypatch: pytest.MonkeyPatch) -> None:
    sync_mock = unittest.mock.Mock()
    monkeypatch.setattr(event.tool_result, "sync", sync_mock)
...
class _RecordingConsolidation(event.ConsolidationEventHandler):
    def __init__(self) -> None:
        super().__init__()
        self.completions: list[models.CompletionEvent] = []
        self.errors: list[models.Error] = []

    def _process_completion(self, event: models.CompletionEvent) -> None:
        self.completions.append(event)

    def _process_error(self, err: models.Error) -> None:
        self.errors.append(err)


def test_consolidation_handler_dispatch() -> None:
...
def test_consolidation_handler_unknown_raises() -> None:
